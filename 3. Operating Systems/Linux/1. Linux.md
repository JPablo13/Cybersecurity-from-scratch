...
The syntax is: [nano (parameter) filename]
- -m: Use the mouse to interact with the text.
- -l: Enables display of the file's line count.
...
- -E: Removes unnecessary whitespace at the end of lines.
...
- **Ctrl+d**: Quits the terminal if there is no text.

#### About Command History
- **Ctrl+p**: Navigates to the previous command in the history.
- **Ctrl+n**: Navigates to the next command in the history.
- **Ctrl+r**: Searches within the history.
- **Ctrl+o**: Executes the command in question within the command history. It is a complement to **Ctrl+r**.

#### About the Terminal Emulator
- **Ctrl+l**: Clears the terminal.
- **Ctrl+s**: Stops scrolling in the terminal output.
- **Ctrl+q**: Resumes scrolling.
- **Ctrl+Alt+(F1-F6)**: Switches between TTI terminals.

## Aliases in Linux
This is a way of assigning a keyword to a custom command. In other words, they allow you to execute a command with certain parameters and arguments, but only through a keyword. The **alias** command displays the commands that have been used. To create an alias, use **alias (alias name = command).** Keep in mind that aliases are temporary; if the system is rebooted or shut down, the created aliases are deleted. To make them permanent, you must add them to the .bashrc file. To delete an alias, use **unalias + alias name**.

## File Types
File types can be identified in the first field when listing files with the **ls** command with the **-l** parameter.
The file types are:
...
| l    | Symbolic links | Points to another file.|
| s    | Socket       | Allows communication between processes. |
| p    | Pipe   | Allows communication between processes. |
| b    | Block file   | Used for communications with the computer (hardware). |
| c    | Character file  | Used for communications with the computer (hardware). |

## Permissions
There are several types of permissions that can be assigned to both files and directories, including read (read), write (write), and execute (execute). Permissions can be assigned in two ways: octal or symbolic.
...
- Group (g): Users who belong to the file/directory group.
...
#### SUID or Set User ID
- **Function**:
	- Allows you to run a file as if it were the owner, regardless of who runs it.
	- It can only be assigned to executable files.
- **Symbol**: (s) replaces the (x) in group permissions.
- **Octal**: Represented by a **4** *Example*: chmod **4**755 filename
...
- **Symbol**: (s) replaces the (x) in the owner's permissions.
- **Octal**: Represented by a **2** *Example*: chmod **2**755 filename
...
- **Octal**: Represented by a **1** *Example*: chmod **1**755 filename.
